...
# Wong W K, Liu C, Deng S, et al. 
# Neighbor group structure preserving based consensus graph learning for incomplete multi-view clustering[J]. 
# Information Fusion, 2023: 101917.
...
Datafold = Dataname + '_percentDel_' + str(percentDel) + '.mat'
...
for iv in range(num_view):
    ...
    Y_iv = Y[iv].T  # 索引从1开始但在Python中从0开始计数
    # 构造相似度矩阵
    Z1 = constructW(Y_iv, options)
    ...
    Sb_ini.append(G[iv].T.dot(Z1).dot(G[iv]))

# 初始化 F_ini 并计算分簇结果
F_ini = solveF(Sk_ini, numClust)
max_iter = 100
[F, Z, obj] = NNGSI_CGL(X[0], Sk_ini, Sb_ini, Wiv, F_ini, numClust, lambda1, lambda2, lambda3, para_r, max_iter)
# 对序列展开以便于聚类
new_F = F.copy()
